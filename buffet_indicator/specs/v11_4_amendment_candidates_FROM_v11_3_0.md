# v11.4 Amendment Candidates — extracted from v11.3.0 verdict.json
...
> This file is RECORD-ONLY — it copies the four amendment candidates from v1.0's
...
## Amendment 1 — Pre-reg §4.1 priors: both monetarist and mean-reversion streams

> *Verbatim from verdict.json `v11_4_amendment_candidates[0]`*:
>
> > Pre-reg §4.1 priors to incorporate both monetarist and mean-reversion literature streams with separate prior probabilities for each component sign

**v11.3.0 context**:

LC v1.0's pre-registration §4.1 priors were calibrated on simple 1970s–90s
monetarist intuition ("loose money → returns"), assigning positive prior
probability to all 5 component betas.

The realized 45-year multi-horizon regression showed the opposite:

- 5-of-5 components have NEGATIVE β at 3Y and 5Y horizons (`z₄ DXY-inv near
  zero at 10Y but still negative; z₁, z₂, z₃, z₅ all clearly negative`)
- LC_DEEP β is robust-negative at 3Y (NW t = −1.89, p = 0.029; 50K-bootstrap
  CI excludes zero) and 5Y (NW t = −1.60, p = 0.055; CI excludes zero)
...
historically PRECEDE lower forward equity returns at end-of-cycle —
...
## Amendment 2 — Criterion 4 sign-vs-magnitude disambiguation
...
LC v1.0 §2.1 Criterion 4 read: "LC_FULL NW t > 1.65 (POSITIVE) any horizon".
The empirical result was: best LC_FULL 3Y horizon shows β = −0.034, t = −3.08
(|t| = 3.08, well above the 1.65 magnitude threshold) — but the sign is
NEGATIVE, opposite to the pre-registered positive direction.

The decision rule treated this as FAIL (since the criterion required β > 0),
...
it's a sign criterion (β > 0 or β < 0 specifically) OR a magnitude criterion
(|β| > X regardless of sign) OR a joint criterion (β > 0 AND |t| > 1.65). No
implicit conjunctions.

---

## Amendment 3 — Non-Gaussian conditional forecast distribution
...
LC v1.0's pre-reg §3 specified Gaussian conditional forecast distributions
...
equity returns (Mandelbrot 1963; Fama 1965; Cont 2001 — fat tails, skew,
volatility clustering).

**v2.0 design implication**: default to skewed-t (Hansen 1994, 4-parameter:
location, scale, skewness η, df ν) OR empirical-kernel distribution. Either
choice should be made in the pre-reg; the chosen family's parameters become
part of the pre-reg invariant.

---

## Amendment 4 — Insufficient-sample gate as explicit pre-reg condition
...
returns is conventionally floor(1.5 × 120) = 180 months — far exceeding the
`5 × HAC_lag = 900-month minimum n_obs` rule of thumb. Session 7's
arbitration noted "n_obs < 5 × HAC_lag → criterion not evaluable as
specified", and the cell was marked FAIL by convention (no positive
evidence) — but this filter was applied **post-hoc**, not pre-registered.

**v2.0 design implication**: list the insufficient-sample gate `n_obs <
5 × HAC_lag` (or analogous bias-correction adequacy rule) in the pre-reg's
§3 methodological-choices section, so future low-n cells are decided
ex-ante rather than re-litigated each sprint.

---

## Cross-reference — full v11.3.0 closeout context

- `LC_V1_SPRINT_CLOSEOUT_REPORT.md` (on `spec/liquidity-composite-v1.0`) — §7 amendment summary
- `outputs/lc_v1_verdict.json` (on `spec/liquidity-composite-v1.0`) — locked scorecard + amendments array (canonical source for this file)
- `DECISIONS.md` (on `spec/liquidity-composite-v1.0`) — 2026-05-24 + 2026-05-25 arbitration entries
- `outputs/reports/lc_v1_research_writeup.md` (on `spec/liquidity-composite-v1.0`) — full research narrative behind the 3 publishable findings
